...
<script>
  ...
  let createSalesRep = () => {
    Inertia.post(route('supervisor.create'), details,{
      onSuccess: () => { jQuery('#addSupervisorModal').modal('hide'); details = { staff_role_id: 2 } }
    });
  }
...
<div class="row">
  <div class="col-sm-12">
    <div class="element-wrapper">
      <h6 class="element-header">Supervisors</h6>
        ...
        <!-- <h5 class="form-header">Powerful Datatables</h5>
          <div class="form-desc">DataTables is a plug-in for the jQuery Javascript library. It is
            ...
            <div class="controls-above-table pb-3">
              <div class="row">
                <div class="col-sm-6">
                  <a class="btn btn-primary btn-md" data-target="#addSupervisorModal" data-toggle="modal"href="#">
            ...
            <div class="table-responsive">
              <table id="dataTable1" width="100%" class="table table-striped table-lightfont">
                ...
                <tbody>
                  {#each supervisors as user (user.id)}
                  <tr>
                    ...
                    <td class="row-actions remove-center">
                      ...
                      <span use:inertia="{{ href: route('supervisor.suspend', user), method: 'put', preserveState: true, preserveScroll:true }}" class="pointer text-orange">
                      ...
                      {#key user.is_suspended}
                      <span  use:inertia="{{ href: route('supervisor.activate', user), method: 'put', preserveState: true, preserveScroll:true }}" class="pointer text-success" href="#">
                        <i class="icon-feather-shield" data-placement="top" data-toggle="tooltip" data-original-title="Activate"></i>
                      </span>
                      {/key}
                      {/if}
                      {#if can_delete}
                      <span on:click="{() => {staffToDelete = user}}" class="text-danger" data-target="#deleteSupervisorModal" data-toggle="modal" href="#"><i class="icon-feather-trash-2" data-placement="top" data-toggle="tooltip" data-original-title="Delete Supervisor"></i></span>
                      {/if}
                    </td>
                  </tr>
                  {/each}
    ...
    <Portal bind:this={staffModals} >
      <div class="onboarding-modal modal fade animated" id="deleteSupervisorModal" role="dialog" tabindex="-1" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-centered" role="document">
          <div class="modal-content text-center"><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span class="close-label">Close</span><span class="os-icon os-icon-close"></span></button>
            ...
            <div class="onboarding-content with-gradient">
              <h4 class="onboarding-title">Delete Supervisor?</h4>
              <div class="onboarding-text"><strong>This action cannot be reversed</strong></div>
              {#if staffToDelete}
              <span  use:inertia="{{ href: route('supervisor.delete', staffToDelete), method: 'delete', preserveState: false, preserveScroll:true, onSuccess:()=>{staffToDelete = null; jQuery('#deleteSupervisorModal').modal('hide'); } }}" class="btn btn-danger btn-md" href="#"><i class="icon-feather-trash-2"></i><span> Delete Supervisor</span></span>
              {/if}
            </div>
          </div>
        </div>
      ...
      <div class="onboarding-modal modal fade animated" id="addSupervisorModal" role="dialog" tabindex="-1" style="display: none;">
        <div class="modal-dialog modal-centered" role="document">
          <div class="modal-content text-center">
            ...
            <div class="onboarding-content with-gradient">
              <h4 class="onboarding-title">Add Supervisor</h4>
              ...
              <form on:submit|preventDefault|stopPropagation="{createSalesRep}">
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <input bind:value={details.user_name} class="form-control" placeholder="Enter login username." type="text" required>
                    </div>
                  </div>
                  <div class="col-sm-6">
                    <div class="form-group">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <input class="form-control" type="file" bind:files on:change="{()=> {details.img = files[0]}}" accept="image/*">
                    </div>
                  </div>
                </div>
                <div class="row">
                ...
                <button class="btn btn-primary btn-md"><i class="icon-feather-plus-circle"></i><span> Create Supervisor Account</span></button>
              </form>
            </div>
          </div>
        </div>
